            ...
            <div className="mt-2 text-sm text-gray-500">
              {metric.status === 'critical' && (
                <div className="mt-1 flex items-center text-error-500">
                  ...
                  <span>Poza normalnym zakresem - skonsultuj się z lekarzem</span>
                </div>
              )}
              ...
              {metric.status === 'warning' && (
                <div className="mt-1 flex items-center text-warning-500">
                  <AlertTriangle className="h-4 w-4 mr-1" />
                  <span>Powyżej zakresu docelowego - monitoruj blisko</span>
                </div>
              )}

              <p className="mt-1 text-xs text-gray-400">
                Zapisano dnia {format(new Date(metric.timestamp), 'd MMM yyyy HH:mm', { locale: pl })}
              </p>
            </div>
          </div>
        ))}
