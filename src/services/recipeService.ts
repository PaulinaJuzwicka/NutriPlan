...
export type Przepis = {
  ...
  kalorie?: number;
  id_kategorii?: number;
  zrodlo?: string; // Zrodlo jako tekst (tytul i autor ksiazki)
  utworzono_o: string;
  zaktualizowano_o: string;
  instrukcje?: string; // Sposób przygotowania
  kategorie?: {
    id: number;
    nazwa: string;
    opis?: string;
...
export const recipeService = {
    ...
    try {
      ...
      if (params.excludeIngredients && params.excludeIngredients.length > 0) {
        recipes = recipes.filter(recipe => {
          ...
          // Sprawdź czy JAKIKOLWIEK ze składników przepisu zawiera wykluczony składnik
  ...
  // Pobieranie kategorii posiłków (typy posiłków)
  async getMealTypes(): Promise<unknown[]> {
    try {
      // Filtrujemy tylko kategorie, które mogą być typami posiłków
      const { data, error } = await supabase
        .from('kategorie')
        .select('*')
  ...
  async updateRecipe(id: number, updates: Partial<Przepis>): Promise<Przepis> {
    try {
      ...
      if (!data) {
        throw new Error('No data returned from recipe update');
      }
