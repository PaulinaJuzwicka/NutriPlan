...
const hasTag = (recipe: IMealRecommendation, tagName: string): boolean => {
  try {
    ...
    if ('suitableFor' in recipe && Array.isArray(recipe.suitableFor)) {
      return recipe.suitableFor.some((item) => item?.toLowerCase() === tagName.toLowerCase());
    }
...
export const getRecommendations = async (): Promise<IMealRecommendation[]> => {
  try {
    return [];
  } catch (error) {
    return [];
